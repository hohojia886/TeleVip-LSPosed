...
object MessageStorage {
    ...
    @JvmStatic
    val storage: Handler = Handler(makeLooper("Storage"))
        ...
        try {
            storage.post {
                ...
                for (i in 0..1) {
                    ...
                    val query = ("SELECT data,mid,uid " +
                            "FROM " + table + " " +
                            "WHERE " + (if (dialogId == 0L) "is_channel" else "uid") + " = " + dialogId + " AND mid IN (" + TextUtils.join(
                        ",",
                        delMsg
                    ) + ");")

                    val update = "UPDATE $table SET data = ? WHERE uid = ? AND mid = ?"
                    val cursor: SQLiteCursor = db.queryFinalized(query, arrayOf())
